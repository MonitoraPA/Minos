...
const createHAR = (page) => {
	...
	for (const [pageIndex, stats] of page.entries()) {
		const pageId = `page_${pageIndex + 1}_${String(Math.random()).slice(2)}`;
		const parsedPage = parsePage(String(pageId), stats);
		har.log.pages.push(parsedPage.page);
		har.log.entries.push(...parsedPage.entries);
	}
	return har;
}
...
function parseHeaders(httpVersion, request, response) {
	...
	if (httpVersion.match(/^http\/[01].[01]$/)) {
		// requestText.length = size of headers of request
...
// return zipped couples = [{key1, value1}, {key2, value2}, etc...]
function zipNameValue(map) {
	const pairs = [];
	for (const [name, value] of Object.entries(map)) { // convert map to array of pairs
